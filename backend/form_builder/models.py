...
class Response(models.Model):
    ...
    def all_answers(self):
        longs = self.long_answers.values(question=F('related_question__question_body'),
                                            answer_type=F('related_question__answer_type'),
                                            answer=F('answer_field'))
        shorts = self.short_answers.values(question=F('related_question__question_body'),
                                            answer_type=F('related_question__answer_type'),
                                            answer=F('answer_field'))
        multi_choices = self.multichoice_answers.values(question=F('related_question__question_body'),
                                                            answer_type=F('related_question__answer_type'),
                                                            answer=F('answer_field__title'))
        emails = self.email_answers.values(question=F('related_question__question_body'),
                                            answer_type=F('related_question__answer_type'),
                                            answer=F('answer_field'))
        phone_nums = self.phonenum_answers.values(question=F('related_question__question_body'),
                                                    answer_type=F('related_question__answer_type'),
                                                    answer=F('answer_field'))
        nums = self.number_answers.values(question=F('related_question__question_body'),
                                            answer_type=F('related_question__answer_type'),
                                            answer=F('answer_field'))
        files = self.file_answers.values(question=F('related_question__question_body'),
                                            answer_type=F('related_question__answer_type'),
                                            answer=F('answer_field'))
...
class LongAnswer(Answer):
    ...
    def save(self, *args, **kwargs):
        if self.related_question.answer_type != QuestionTypes.Long:
            raise ValueError(f'answer type has to be {self.related_question.answer_type}, but Long Answer was given!')
        return super().save(*args, **kwargs)
